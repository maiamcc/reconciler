...
def test_transaction(date='01/01/2000', payee='Some Merchant', inflow: float = 0, outflow: float = 0, notes=''):
...
class TestCollapseTransactions:
    def test_collapse(self):
        transactions = [
            test_transaction(date='04/20/1969', payee='Some Merchant', inflow=0, outflow=1.11),
            test_transaction(date='04/20/1969', payee='Some Merchant', inflow=0, outflow=2.22),
            test_transaction(date='04/20/1969', payee='Some Merchant', inflow=0, outflow=3.33),
        ]
        ...
        assert collapsed.outflow == 6.66
        assert '3 transactions' in collapsed.notes

    def test_collapse_inflow(self):
        transactions = [
            test_transaction(date='04/20/1969', payee='Some Merchant', inflow=1.11, outflow=0),
            test_transaction(date='04/20/1969', payee='Some Merchant', inflow=2.22, outflow=0),
            test_transaction(date='04/20/1969', payee='Some Merchant', inflow=3.33, outflow=0),
        ]
        ...
        assert collapsed.date == _date_from_str('4/20/1969')
        assert collapsed.payee == 'Some Merchant'
        assert collapsed.inflow == 6.66
        assert collapsed.outflow == 0
        assert '3 transactions' in collapsed.notes

    def test_multiple_payees(self):
        transactions = [
            test_transaction(date='04/20/1969', payee='FooCo.', outflow=1.11),
            test_transaction(date='04/20/1969', payee='Bar Shop', outflow=2.22)
        ]
        ...
        assert collapsed.inflow == 0
        assert collapsed.outflow == 3.33
        assert '2 transactions' in collapsed.notes

    def test_multiple_dates_throws_error(self):
        transactions = [
            test_transaction(date='04/20/2040', payee='FooCo.', outflow=1.11),
            test_transaction(date='04/20/1969', payee='Bar Shop', outflow=2.22)
        ]
...
class TestNormalizeYnabSplits:
    def test_noop(self):
        transactions = [
            test_transaction(date='04/20/1969', payee='Foo', outflow=1.11),
            test_transaction(date='04/21/1969', payee='Bar', outflow=2.22),
            test_transaction(date='04/22/1969', payee='Baz', inflow=3.33)
        ]
    ...
    def test_normalize_splits(self):
        transactions = [
            test_transaction(date='04/20/1969', payee='Foo', outflow=1.11),
            test_transaction(date='04/21/1969', payee='Bar', outflow=2.22, notes='Split (1/2)'),
            test_transaction(date='04/21/1969', payee='Bar', outflow=0.44, notes='Split (2/2)'),
            test_transaction(date='04/22/1969', payee='Baz', inflow=3.33),
            test_transaction(date='04/23/1969', payee='Beep', inflow=0.01, notes='Split (1/3)'),
            test_transaction(date='04/23/1969', payee='Beep', inflow=0.02, notes='Split (2/3)'),
            test_transaction(date='04/23/1969', payee='Beep', inflow=0.03, notes='Split (3/3)'),
            test_transaction(date='04/24/1969', payee='Quux', outflow=123),
        ]
